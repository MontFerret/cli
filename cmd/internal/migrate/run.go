...
func newRunCommand(store *config.Store, service Service) *cobra.Command {
	command := &cobra.Command{
		...
		Long: "Run supported Ferret v1-to-v2 migrations on a standalone FQL file or project directory. " +
			...
			"It also migrates safe legacy stdlib calls to encoding, crypto, path, arrays, object, datetime, math, and random namespaces. " +
			"Literal modes and composed replacements are supported, including sorted_unique, shift, and sorted keys. " +
			"Ambiguous join, unsafe array modes, nonfloating date_diff, date_compare, aggregate math, parameterized rand, and calls affected by " +
			"local function or use alias collisions are left for manual follow-up. " +
			"Range and zero-argument rand are migrated. Already-qualified calls are preserved. " +
