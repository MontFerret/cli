...
func TestModuleCommandSearchRendersTableAndEmptyResult(t *testing.T) {
	...
	output, err := executeModuleCommand(t, service, "search", "sqlite")
	if err != nil {
		t.Fatal(err)
	}
	...
	output, err = executeModuleCommand(t, service, "search")
	if err != nil {
		t.Fatal(err)
	}
...
func TestModuleCommandInfoRendersOptionalFields(t *testing.T) {
	...
	output, err := executeModuleCommand(t, service, "info", "acme/sqlite")
	if err != nil {
		t.Fatal(err)
	}
...
func TestModuleCommandCreateRequiresAndPassesFlags(t *testing.T) {
	service := &fakeModuleService{create: &modulelifecycle.CreateResult{Directory: "/tmp/sqlite", Namespace: "DB::SQLITE"}}
	if _, err := executeModuleCommand(t, service, "create", "db/sqlite"); err == nil || !strings.Contains(err.Error(), "--go-module is required") {
		t.Fatalf("unexpected missing flag error: %v", err)
	}

	output, err := executeModuleCommand(t, service,
		"create", "db/sqlite",
...
func TestModuleCommandPublishRendersRecords(t *testing.T) {
	...
	output, err := executeModuleCommand(t, service, "publish", "--tag", "release-1.2.3")
	if err != nil {
		t.Fatal(err)
	}
...
func TestModuleCommandExposesOnlyLifecycleSubcommands(t *testing.T) {
	command := ModuleCommand(newConfigCommandTestStore(t, t.TempDir()), &fakeModuleService{})
	want := map[string]bool{"search": true, "info": true, "create": true, "publish": true}
	for _, child := range command.Commands() {
		delete(want, child.Name())
		if child.Name() == "add" || child.Name() == "install" || child.Name() == "update" {
...
func TestModuleCommandPropagatesServiceErrors(t *testing.T) {
	want := errors.New("registry unavailable")
	_, err := executeModuleCommand(t, &fakeModuleService{err: want}, "search")
	if !errors.Is(err, want) {
		t.Fatalf("expected service error, got %v", err)
	}
}

func executeModuleCommand(t *testing.T, service moduleService, args ...string) (string, error) {
	t.Helper()

	command := ModuleCommand(newConfigCommandTestStore(t, t.TempDir()), service)
