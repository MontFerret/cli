...
func TestServiceSearchPropagatesBarnAndCancellationErrors(t *testing.T) {
	t.Run("typed Barn error", func(t *testing.T) {
		...
		service := NewService(&fakeRegistry{searchErr: failure}, nil, nil)
	...
	t.Run("cancellation", func(t *testing.T) {
		service := NewService(&fakeRegistry{search: func(ctx context.Context, _ barnregistry.SearchOptions) ([]barnregistry.ModuleSummary, error) {
			<-ctx.Done()
...
func TestServiceInfoPropagatesBarnErrors(t *testing.T) {
	service := NewService(&fakeRegistry{moduleErr: barnregistry.ErrModuleNotFound}, nil, nil)
	...
	service = NewService(&fakeRegistry{modules: map[string]*barnregistry.Module{
